...
public class Queen extends Figure {
     ...
     * @throws ImpossibleMoveException Queen can't move
     */
    public Cell[] way(Cell dist) throws ImpossibleMoveException {
        ...
        if (Math.abs(x) == Math.abs(y)) {
            ...
            throw new ImpossibleMoveException("Queen can't move to this destination cell");
        }
        return cells;
    }
    /**
    ...
    public void clone(Cell dist) {
        ...
        for (int i = 0; i < temp.length; i++) {
            if (temp[i].equals(this.position.getFigure())) {
                temp[i] = new Queen(dist, this.color);
            }
        }
